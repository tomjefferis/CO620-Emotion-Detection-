...
#--- APPLE_MLCOMPUTE ---
        if mlcompute.enable_mlc_op(input_tensors=[inputs[0]], eager=True):
            output_size = (
                self.output_size if self.output_size is not None
                else self.value_kernel.shape[1]
            )
            # MLC restriction.
            if (output_size == self.key_kernel.shape[1] and
                output_size == self.value_kernel.shape[1] and
                output_size == self.query_kernel.shape[1] and
                (self.num_heads * self.head_size == output_size)):
                if self.projection_bias is None:
                    projection_bias = K.constant(
                        0.0, dtype=self.dtype, shape=[output_size])
                else:
                    projection_bias = self.projection_bias
                if mask is None:
                    mask = K.constant(
                        0.0, dtype=tf.float32,
                        shape=[1, query.shape[-2], key.shape[-2]])
                else:
                    mask = tf.cast(mask, tf.float32)
                    if len(mask.shape) != len(query.shape):
                        mask = tf.expand_dims(mask, -3)
                    mask = mask[0:1, :, :]
                return gen_mlc_ops.mlc_multi_head_attention(
                    query,
                    key,
                    value,
                    mask,
