...
#--- APPLE_MLCOMPUTE ---
from tensorflow.compiler.tf2mlcompute.ops import gen_mlc_ops
...
#--- APPLE_MLCOMPUTE ---
        ...
        # Adjust self.axis to positive value if needed.
        if self.axis < 0:
            self.axis += ndims
#--- APPLE_MLCOMPUTE ---
        self.built = True
...
#--- APPLE_MLCOMPUTE ---
        if (mlcompute.enable_mlc_op(eager=True, input_tensors=[inputs]) and
            # Only support 2D norm for now. Explore 1D if there is a use case.
            inputs.shape.rank == 4):
            dim = input_shape[self.axis]
            param_shape = [1, 1, 1, dim] if self.axis == 3 else [1, dim, 1, 1]
            if not self.center:
                self.beta = K.constant(
                    0.0, dtype=self.dtype, shape=param_shape)
            if not self.scale:
                self.gamma = K.constant(
                    1.0, dtype=self.dtype, shape=param_shape)
            data_format = "NHWC"
            if self.axis == 1:
                data_format = "NCHW"
            if ((self.axis == 3 or self.axis == 1) and
                (self.groups == input_shape[self.axis])):
                return gen_mlc_ops.mlc_instance_norm(
                    inputs, self.gamma, self.beta, self.epsilon, data_format)
            return gen_mlc_ops.mlc_group_norm(
                inputs, self.gamma, self.beta, epsilon=self.epsilon,
                data_format=data_format, groups=self.groups)
#--- APPLE_MLCOMPUTE ---
